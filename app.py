    ...
    if show_splits:
        ...
        if not splits.empty:
            ...
            for _, row in splits_in_range.iterrows():
                # Converteer datum naar string voor Plotly-compatibiliteit in subplots
                split_date = pd.to_datetime(row["date"]).strftime("%Y-%m-%d")
                # Gebruik add_shape ipv add_vline (laatste heeft bug bij subplots + datum)
                fig.add_shape(
                    type="line",
